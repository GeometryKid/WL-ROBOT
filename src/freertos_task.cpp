...
TaskHandle_t bleCheckTaskHandle = NULL;
TaskHandle_t xboxTaskHandle = NULL;
TaskHandle_t ChassisTaskHandle = NULL;

uint8_t xbox_datas[28] = {0};
uint16_t CRC16_Table(uint8_t *p, uint8_t counter);
HardwareSerial to_stm32_serial(1);
XboxSeriesXControllerESP32_asukiaaa::Core
    xboxController("28:ea:0b:ff:97:ae");
// 串口发送任务，200Hz，绑定到 Core 0
...
void uartTask(void *pvParameters)
{
    ...
    for (;;)
    {
        send_serial_frame_mat(0x01, 28, xbox_datas);
        vTaskDelay(xFrequency); // 使用相对延时，延时 5ms
    }
}
...
void create_freertos_tasks()
{
    ...
    to_stm32_serial.begin(115200, SERIAL_8N1, 4, 5); // rx tx
...
void send_serial_frame_mat(uint8_t frame_id, uint8_t data_length, uint8_t *data)
{
    ...
    buff_msg[4 + data_length] = tx_frame_mat.check_code.crc_buff[0];
    buff_msg[5 + data_length] = tx_frame_mat.check_code.crc_buff[1];
    ...
    to_stm32_serial.write(buff_msg, sizeof(buff_msg));
}
